...
processed_abstracts <- textProcessor(data$paperabstract, metadata = data) 
out_abstracts <- prepDocuments(processed_abstracts$documents, processed_abstracts$vocab, processed_abstracts$meta)
docs_abstracts <- out_abstracts$documents
vocab_abstracts <- out_abstracts$vocab
meta_abstracts <- out_abstracts$meta

#Prepare data
plotRemoved(processed_abstracts$documents, lower.thresh = seq(1, 200, by = 100))
out_abstracts <- prepDocuments(processed_abstracts$documents, processed_abstracts$vocab, processed_abstracts$meta, lower.thresh = 15)

#Estimating the structural topic model
poliblogPrevFit_abstracts <- stm(documents = out_abstracts$documents, 
                              vocab = out_abstracts$vocab, 
                              K = 20, 
                              prevalence = ~publication_year + country, 
                              max.em.its = 75, 
                              data = out_abstracts$meta, 
                              init.type = "Spectral")

poliblogPrevFit_abstracts
...
poliblogSelect_titles <- selectModel(out_titles$documents, 
                              out_titles$vocab, 
                              K = 20, 
                              prevalence = ~publication_year + country, 
                              max.em.its = 10, 
